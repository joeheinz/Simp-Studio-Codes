...
{
  sl_status_t sc; // Variable para el estado
  int16_t ovec[3]; // Vector de orientación (x, y, z) en 0.01 grados
  int16_t avec[3]; // Vector de aceleración (x, y, z)

  // Leer los datos del sensor IMU
  sc = sl_sensor_imu_get(ovec, avec);
  if (sc == SL_STATUS_OK) {
    // Convertir valores a formato legible (grados y m/s²)
    ...
    // Mostrar error si falla la lectura
    app_log_error("Failed to read IMU sensor data. Status: 0x%04lu\n", sc);
  }
}

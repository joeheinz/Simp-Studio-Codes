 ...
 *
 * I did this, nigga.
 * TODO: Implementate the functions nigga, use fent
 */
//Updates the IMU characteristi
...
SL_WEAK void app_init(void)
{
  ...
  app_log_error("Niggatron, it was everything initialized.");
  /////////////////////////////////////////////////////////////////////////////
...
SL_WEAK void app_process_action(void)
{
  ...
  if (report_button_flag) {
    sl_status_t sc;
    app_log_error("The report button flag was triggered (button)");
  ...
  ///
  /////////////////////////////////////////////////////////////////////////////
  // Nuevo Bloque: Leer datos del sensor IMU cada 100 ms                      //
  /////////////////////////////////////////////////////////////////////////////

  static uint32_t last_read_time = 0;
    uint32_t current_time = sl_sleeptimer_get_tick_count();

    // Read IMU data every 100 ms
    if ((current_time - last_read_time) >= sl_sleeptimer_ms_to_tick(100)) {
      last_read_time = current_time;
      app_log_error("NIGGER");
      // Update and notify IMU data
      sl_status_t floyd = update_report_imu_characteristic();
      app_log_status_error(floyd);
...
static sl_status_t update_report_imu_characteristic(void)
{
  ...
  sc = sl_bt_gatt_server_write_attribute_value(gattdb_imu_acceleration,
                                               ...
                                               data_send);
  if (sc == SL_STATUS_OK) {
    app_log_info("IMU data written to GATT: X=%d, Y=%d, Z=%d\n",
                 avec[0], avec[1], avec[2]);
  } else {
    app_log_error("Failed to write IMU data to GATT. Status: 0x%04X\n", sc);
  }
  return sc;
}

// Send IMU notification
static sl_status_t send_report_imu_notification(void)
{
  sl_status_t sc;
  int16_t avec[3];
  ...
  if (sc == SL_STATUS_OK) {
    ...
    app_log_error("Failed to send IMU notification. Status: 0x%04X\n", sc);
  }
  return sc;
}
...
void get_imu_sensor_data(void)
{
  sl_status_t sc; // Status variable
  int16_t ovec[3]; // Orientation vector
  int16_t avec[3]; // Acceleration vector

  // Read IMU data
  sc = sl_sensor_imu_get(ovec, avec);
  if (sc == SL_STATUS_OK) {
    // Convert data
    float orientation_x = ovec[0] * 0.01f;
    float orientation_y = ovec[1] * 0.01f;
    float orientation_z = ovec[2] * 0.01f;

    float accel_x = avec[0] * 0.01f;
    float accel_y = avec[1] * 0.01f;
    float accel_z = avec[2] * 0.01f;

    // Log the values
    app_log_info("Orientation (deg): X: %.2f, Y: %.2f, Z: %.2f\n",
                 orientation_x, orientation_y, orientation_z);
    app_log_info("Acceleration (m/s^2): X: %.2f, Y: %.2f, Z: %.2f\n",
                 accel_x, accel_y, accel_z);

    // Prepare IMU data for notification
    uint8_t imu_data[12];
    memcpy(imu_data, avec, sizeof(avec)); // Copy acceleration data

    // Send IMU data as notification via the button GATT
    sc = sl_bt_gatt_server_notify_all(gattdb_imu_acceleration,
                                      sizeof(imu_data),
                                      imu_data);
    if (sc != SL_STATUS_OK) {
      app_log_error("Failed to send IMU notification. Status: 0x%04lx\n", sc);
    }
  } else {
    app_log_error("Failed to read IMU sensor data. Status: 0x%04lx\n", sc);
  }
}
    ...
    case sl_bt_evt_gatt_server_characteristic_status_id:
      if (gattdb_report_button == evt->data.evt_gatt_server_characteristic_status.characteristic) {
        ...
        if (evt->data.evt_gatt_server_characteristic_status.client_config_flags
          ...
          app_log_info("Notification enabled, nigga.");
...
static sl_status_t update_report_button_characteristic(void)
{
  ...
  sc = sl_bt_gatt_server_write_attribute_value(gattdb_report_button,
                                               ...
                                               &data_send);
  if (sc == SL_STATUS_OK) {
    app_log_info("Attribute written, nigga: 0x%02x", (int)data_send);
  }
...
static sl_status_t send_report_button_notification(void)
{
  ...
  sc = sl_bt_gatt_server_notify_all(gattdb_report_button,
                                    ...
                                    &data_send);
  if (sc == SL_STATUS_OK) {
    app_log_append(" Notification also sent to niggatron: 0x%02x\n", (int)data_send);
  }
  return sc;
}
...
static sl_status_t update_report_imu_characteristic(void)
{
  sl_status_t floyd;  // Status variable
  int16_t avec[3];    // Acceleration data (X, Y, Z)
  uint16_t ovec[3]; // 6 bytes (2 bytes per axis: X, Y, Z)

  // Read IMU acceleration data
  floyd = sl_sensor_imu_get(ovec, avec); // Only acceleration data
  if (floyd != SL_STATUS_OK) {
    app_log_error("Failed to read IMU data. Status: 0x%04lu\n", floyd);
    return floyd;
  }


  // Write the IMU data to the GATT database
  floyd = sl_bt_gatt_server_write_attribute_value(gattdb_imu_acceleration, // Replace with your GATT characteristic
                                                  0,
                                                  sizeof(avec),
                                                  (uint8_t*)avec);

  if (floyd == SL_STATUS_OK) {
    app_log_info("IMU Data Written: X=%d, Y=%d, Z=%d\n",
                 avec[0], avec[1], avec[2]);
  } else {
    app_log_error("Failed to write IMU data. Status: 0x%04lu\n", floyd);
  }
...
static sl_status_t send_report_imu_notification(void)
{
  ...
  // Read IMU acceleration data from GATT
  floyd = sl_bt_gatt_server_read_attribute_value(gattdb_imu_acceleration, // Replace with your GATT handle
                                              0,
                                              sizeof(avec),
                                              &data_len,
                                              (uint8_t*)avec);
  ...
  // Send notification for acceleration data
  floyd = sl_bt_gatt_server_notify_all(gattdb_imu_acceleration,
                                    sizeof(avec),
                                    (uint8_t*)avec);
  if (floyd == SL_STATUS_OK) {
    app_log_info("IMU acceleration notification sent.\n");
  } else {
    app_log_error("Failed to send IMU acceleration notification.\n");
  }
